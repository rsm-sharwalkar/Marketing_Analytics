...
```{python}
table4_df = df[df["demp"].notna()].copy()
table4_df = table4_df[
    (table4_df["closed"] == 1) |
    ((table4_df["closed"] == 0) & (table4_df["dwage"].notna()))
].copy()

model1 = smf.ols("demp ~ gap", data=table4_df).fit()

pd.DataFrame({
    "Coefficient": [model1.params["gap"]],
    "Std. Error": [model1.bse["gap"]]
}, index=["gap"]).round(3)
```
